...
namespace FlashStart
{
    class FileHelper
    {
        ...
        public static void CopyFile(string sourcePath, string directionPath,bool isCover)
        {
            //File.Copy(@"D:\Test\Debug1\测试.txt", @"D:\Test\Debug2\测试1.txt", true);
            //参数1：指定要判断的文件路径
            if (!Directory.Exists(Path.GetDirectoryName(directionPath)))
            {
            ...
            if (!File.Exists(directionPath))
            {
                ...
                FileStream fs = new FileStream(directionPath, FileMode.Create, FileAccess.ReadWrite);
                fs.Close();
            }
            File.Copy(sourcePath, directionPath, isCover);
        }
    }
}
